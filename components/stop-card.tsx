...
interface StopCardProps {
  ...
  isFirstStopOfTrip: boolean // Add new prop
}

const stopTypes = [
  { name: "Default", icon: <div className="w-4 h-4 rounded-full bg-orange-500 border-2 border-white" /> },
  { name: "Overnight", icon: <Moon className="w-4 h-4 text-blue-800" /> },
  { name: "Outdoor Activity", icon: <TreePine className="w-4 h-4 text-green-600" /> },
  { name: "Place of Interest", icon: <CameraIcon className="w-4 h-4 text-teal-500" /> },
  { name: "Fuel", icon: <Fuel className="w-4 h-4 text-red-500" /> },
  { name: "Food and Drink", icon: <Utensils className="w-4 h-4 text-orange-500" /> },
]
...
export const StopCard: FC<StopCardProps> = ({ stop, dayId, stopNumber, onDeleteStop, isFirstStopOfTrip }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className={isDragging ? "opacity-50" : ""}>
      {showDrivingDetails && <DrivingDetails details={stop.driving} />}
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <div className="border rounded-lg bg-background">
          <div className="flex items-center gap-1 p-2">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onDeleteStop(dayId, stop.id)}>
            ...
            <div className="p-4 pt-0 space-y-4">
              <div className="flex gap-2">
                <div className="flex items-center border rounded-md overflow-hidden">
                  <span className="px-3 py-2 bg-gray-100 border-r text-sm font-medium text-muted-foreground">£</span>
                  <Input
                    type="text"
                    defaultValue="0"
              ...
              <div className="space-y-2">
                ...
                <Input id={`custom-name-${stop.id}`} className="w-full" placeholder="Enter custom name" />
              </div>
              <div className="space-y-2">
