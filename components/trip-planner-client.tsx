...
import { Calendar, MapPin, Route, Clock, Globe, DollarSign, Loader2 } from "lucide-react"
...
export function TripPlannerClient({ initialTripData, tripId }: TripPlannerClientProps) {
  const [tripData, setTripData] = useState<TripWithSettings | null>(initialTripData)
  ...
  const handleUpdateSettings = async (newSettings: Omit<Settings, "id" | "tripId">) => {
    if (!tripData) return
    const previousTripData = tripData
    const updatedSettings = { ...tripData.settings, ...newSettings }
    setTripData({ ...tripData, settings: updatedSettings })
  ...
  const handleTripDetailsChange = async (data: { name?: string; dates?: string }) => {
    if (!tripData) return
    const previousTripData = tripData
  ...
  const handleDateRangeChange = (newDateRange: DateRange | undefined) => {
    if (!tripData || !newDateRange?.from || !newDateRange?.to) return
    const newDates = `${format(newDateRange.from, "dd/MM/yyyy")} - ${format(newDateRange.to, "dd/MM/yyyy")}`
    handleTripDetailsChange({ dates: newDates })
  }

  const stats = useMemo(() => {
    if (!tripData) return { days: 0, stops: 0, distance: 0, hours: 0, countries: 1, cost: 0 }
    let totalStops = 0,
      totalDistance = 0,
      totalHours = 0,
      totalCost = 0
    tripData.days.forEach((day) => {
      ...
      day.stops.forEach((stop) => {
        if (stop.driving) {
          ...
          if (minutesMatch) totalHours += Number.parseInt(minutesMatch[1], 10) / 60
        }
      })
    })
    return {
  ...
  if (!googleMapsApiKey) {
    return (
      <div className="flex items-center justify-center h-screen text-red-500">
        <p>Google Maps API key is missing. Please add NEXT_PUBLIC_GOOGLE_MAPS_API_KEY to your environment variables.</p>
      </div>
    )
  }
    ...
    <>
      <div className="flex h-screen bg-background">
        <div className="w-96 border-r bg-background flex flex-col">
          ...
          <div className="p-4 border-b">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground">{tripData.settings.distanceUnit}</div>
              </div>
              <div className="text-center">
          ...
          <TripSidebar trip={tripData} setTripData={setTripData} />
        </div>
        <div className="flex-1 relative">
          <TripMap
            mapStyle={tripData.settings.mapStyle}
            stops={tripData.days.flatMap((day) => day.stops)}
        ...
        />
        <ShareModal open={showShare} onOpenChange={setShowShare} tripName={tripData.name} />
      </div>
      <Toaster />
    </>
  )
